...
class VectorDbManager:
    ...
    def __init__(self):
        ...
        self.__dense_embeddings = HuggingFaceEmbeddings(model_name=self.settings.VECTORSTORE_DENSE_MODEL)
        self.__sparse_embeddings = FastEmbedSparse(model_name=self.settings.VECTORSTORE_SPARSE_MODEL)
        ...
        if not self.__client.collection_exists(collection_name):
            ...
            if existing_size and existing_size != expected_size:
                raise ValueError(
                    ...
                    f"{existing_size}, but '{self.settings.VECTORSTORE_DENSE_MODEL}' produces size "
